...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
                ...
                for (var i = 0; i < columns.length; i++) {
                    if (columns[i].format) {
                        $('#' + dataId).find('tr').each(function () {
                            ...
                            var lineobj = $field.data("lineobj");
                            $field.html(columns[i].format($field.text(),lineobj));
                        });
                    }
                }
    ...
    function getTpl(options) {
        ...
        var tpl = '{{# if(d.rows.length>0 && d.rows!=undefined){ }}';
        tpl += '{{# layui.each(d.rows, function(index, item){ }}';
        var tds = '';
        for (var i = 0; i < columns.length; i++) {
            if(columns[i].format){
                tds += '<td data-field="' + columns[i].field + '" data-lineobj=\'{{JSON.stringify(item)}}\'>{{ item.' + columns[i].field + ' }}</td>';
            }else{
